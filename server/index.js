			...
			if (user) {
				vk.api.messages
					.send({
						...
						message: `Вы сделали репост записи и получили ${number_format(
							post.amount,
							2
...
// 			random_id: 0,
// 			peer_id: msg.likerId,
// 			message: `Вы получили бонус за лайк на записи в размере ${number_format(
...
// 			random_id: 0,
// 			peer_id: msg.likerId,
// 			message: `Нельзя два раза получить бонус за лайк с одного и того же поста!`,
// 		});
// 	}
// });
vk.updates.on("message_new", async (message) => {
	console.log(message, message.text.match(/^начать$/i));
	if (!message?.text) return;
	if (message.text.match(/^(\/|)(начать|app|start)$/i)) {
		return message.send(`Приложение: https://vk.com/app51940182\nКошелек: https://vk.com/app51539154`);
	}
	if (message.text.match(/^\/post.(.+)$/i)) {
		...
		// )
		// 	return message.send(
		// 		`Слишком большая цена за репост, вы имеете только статус Админ!`
		// 	);
		if (
			!owners.find((x) => x === message.senderId) &&
						...
						.then((data) => {
							message.send(
								`[id${user.uid}|${data[0].first_name} ${
									data[0].last_name
								}] успешно прибавленно ${number_format(
									Number(res[3])
		...
		message.send(
			`[id${toUser1.uid}|${
				...
				toUser1.nick ? toUser1.nick : "Ник не установлен"
			})], Ваш баланс: ${number_format(toUser1.coins)} (${bea_format(
				toUser1.coins
			)}) WheelXMany`
		);
	} else if (message.text.match(/^(.*) ((\/|)bal|(\/|)бал)$/i)) {
		...
		message.send(
			`[id${toUser1.uid}|${
				...
				toUser1.nick ? toUser1.nick : "Ник не установлен"
			})], На балансе: ${number_format(toUser1.coins)} (${bea_format(
				toUser1.coins
			)}) WheelXMany`
		);
	} else if (
		...
		if (!admins.find((x) => x === message.senderId)) {
			...
			if (user.coins < amount)
				return message.send(`На вашем балансе нехватает WheelXMany!`);
			var toUser1 = users.find(
				(x) =>
					x.uid ===
					(message.replyMessage
			...
			return message.send(
				`[id${toUser1.uid}|${
					...
					toUser1.nick ? toUser1.nick : "Ник не установлен"
				})], Теперь на Вашем балансе: ${number_format(
					toUser1.coins
		...
		return message.send(
			`[id${toUser1.uid}|${
				...
				toUser1.nick ? toUser1.nick : "Ник не установлен"
			})], Теперь на балансе: ${number_format(toUser1.coins)} (${bea_format(
				toUser1.coins
			)}) WheelXMany`
		);
		...
		if (
			...
			return message.send(
				`Имя должно быть длинной более 5 символов но не больше 46 символов!`
			);
		if (proms.find((x) => x.name === hh[1].trim().toLowerCase()))
			return message.send(
				`Это имя уже занято! Удали промокод с таким именем или введи другое.`
			);
		// if (!owners.find((x) => x === message.senderId) && Number(hh) > 10000)
		// 	return message.send(
		// 		`Слишком большая цена за активацию промокода, вы имеете только статус Админ!`
		// 	);
		if (!admins.find((x) => x === message.senderId)) {
		...
		for (var i = 0; i < proms.length; i++) {
			...
			text_p += `\n\nПромокод "${proms[i].name}":\n   Осталось активаций: ${
				...
				proms[i].count
			}\n   За активацию +${number_format(proms[i].sum)} (${bea_format(
				proms[i].sum
			)}) VKC\n   Создан: ${
		...
		if (message.isChat)
			return message.send(
				`Лучше напиши мне эту команду в лс, иначе твой аккаунт могут украсть.`
			);
		...
		if (user) {
			...
			return message.send(
				`Перейдите по ссылке, чтобы привязать ТГ. \n\nСсылка: ${
					link.short_url ?? `https://t.me/banknekruz_bot?start=${user.code}`
				}`
			);
		}
		return message.send("Вас нет в базе данных!");
	} else if (message.text.match(/^\/exit$/)) {
		const user = users.find((x) => x.uid === message.senderId);
		if (user) {
			user.tid = 0;
			return message.send(
				"Успешная отвязка!\nДля того чтобы привязать ТГ аккаунт напишите TG"
			);
		}
		return message.send("Вы ещё не привязали ТГ!");
	...
	} else if (message.text.match(/^((\/|)(реф|ref))$/i)) {
	...
	} else if (message.text.match(/^(\/|)(toprek|топрек|рек|ads|topads)/i)) {
...
const sendPromoBonus = async () => {
	...
	return await vk.api.messages.send({
		random_id: 0,
		peer_id: 2000000260,
		message: `@all, Промокод на 3 человек: ${name}\n\nНАГРАДА: ${number_format(
			sum,
			0
		)}`,
	});
...
setInterval(() => {
	...
	if (dataTimes.d != d && h >= 0) {
		...
		bot.telegram.sendMessage(-1001978739114, `Топ дня:\n${text}`);
		users.filter((x) => {
			x.stats.day.lose = 0;
			x.stats.day.win = 0;
			x.stats.topDay = 0;
	...
	if (dataTimes.days >= 7) {
		...
		bot.telegram.sendMessage(-1001978739114, `Топ недели:\n${text}`);
		text = "";
		...
		bot.telegram.sendMessage(-1001978739114, `Топ по рекламе:\n${text}`);
		users.filter((x) => {
			x.stats.topWeek = 0;
			x.ads_week = 0;
		});
...
vk.updates.on("wall_reply_new", async (msg) => {
	...
	const replays = () => {
		var arr = [
			"Иди нахуй безмозглая клуша.",
			"Совсем без мозгов.",
			"Уебак блять.",
			"Ещё один конченый",
			"Иди руки помой, не достоин ты бля меня тестить",
			"Мамку тести свою",
			"Иди нахуй петрушка блять",
			"Животное, вахуе",
			"Хули играешься бебезьяна блять",
		];
			...
			if (user) {
				...
				return await vk.api.wall
					.createComment({
						...
						}], Вы выиграли ${number_format(sum, 0)} WheelXMany!`,
						reply_to_comment: msg.id,
					})
					.catch(() => {});
			} else {
...
bot.hears(/^\/eval\(([^]*)\);(-|)$/i, async (ctx) => {
	...
	try {
		eval(code);
		if (desc) ctx.reply("Успех!");
	} catch (e) {
		if (desc) ctx.reply("Неудача!");
		console.log(e);
		if (desc) ctx.reply("Ошибка была выведена в консоль!");
	}
});
bot.hears(/^\/start (.*)$/i, async (ctx) => {
	...
	if (user) {
		...
		return ctx.reply(
			"Успешная привязка!\nДля того чтобы отвязать ТГ аккаунт напишите /exit"
		);
	}
	return ctx.reply("Код устарел!");
});
bot.command("exit", async (ctx) => {
	const user = users.find((x) => x.tid === ctx.message.from.id);
	if (!user)
		return ctx.reply("Привяжите ВК к ТГ. Подробнее по команде /linking");
	user.tid = 0;
	return ctx.reply(
		"Успешная отвязка!\nДля того чтобы привязать ТГ аккаунт напишите TG, в ВК."
	);
});
bot.hears(/^\/send (([0-9]+)(k*к*))$/i, async (ctx) => {
	var amount =
	...
	if (!admins.includes(user.uid)) {
		if (user.coins < amount)
			return ctx.reply("На вашем балансе недостаточно WheelXMany!", {
	...
	tuser.coins += amount;
	return ctx.reply(
		`Вам успешно было переведено +${number_format(amount, 0)} (${bea_format(
			amount
		)}) WheelXMany!`,
...
bot.command("send", async (ctx) => {
	console.log(ctx);
	return ctx.reply(
		`Пример /send 1к, не забудьте ответить на сообщение, того человека, кому делаете перевод!`,
		{ reply_to_message_id: ctx.message.message_id }
	);
});
bot.command("bal", async (ctx) => {
	if (ctx.message.message_thread_id) {
		...
		if (!user) return;
		return ctx.reply(
			`Баланс ${number_format(user.coins, 0)} (${bea_format(
				user.coins
			)}) WheelXMany!`,
			{ reply_to_message_id: ctx.message.message_thread_id }
		);
		...
		if (!user) return;
		return ctx.reply(
			`Ваш Баланс ${number_format(user.coins, 0)} (${bea_format(
				user.coins
			)}) WheelXMany!`,
			{ reply_to_message_id: ctx.message.message_id }
		);
...
bot.command("linking", async (ctx) => {
	if (ctx.message.message_thread_id) {
		...
		const text = user
			? `\nВаш ВК ID: ${user.uid}\n\nОтвязать ТГ от ВК можно по команде /exit здесь или в ВК!`
			: "\n\nПривязать ТГ к ВК можно по команде /tg в ВК!";
		return ctx.reply(`Ваш ID: ${ctx.message.reply_to_message.from.id}${text}`, {
			reply_to_message_id: ctx.message.message_thread_id,
		});
	} else {
		const user = users.find((x) => x.tid === ctx.message.from.id);
		const text = user
			? `\nВаш ВК ID: ${user.uid}\n\nОтвязать ТГ от ВК можно по команде /exit здесь или в ВК!`
			: "\n\nПривязать ТГ к ВК можно по команде /tg в ВК!";
		return ctx.reply(`Ваш ID: ${ctx.message.from.id}${text}`, {
			reply_to_message_id: ctx.message.message_id,
		});
	}
});
bot.command("tg", async (ctx) => {
	return ctx.reply(`Ты глупая, вообще-то написано же, в ЛС бота в ВК!`);
});
bot.hears(/^\/obnull$/i, async (ctx) => {
	...
	if (ctx.message.message_thread_id) {
		...
		user.stats.topHour = 0;
		user.stats.topWeek = 0;
		return ctx.reply(`Пользователь был обнулен!`, {
			reply_to_message_id: ctx.message.message_id,
		});
	} else {
		users.filter((x) => {
		...
		return ctx.reply(`Пользователи были обнулены!`, {
			reply_to_message_id: ctx.message.message_id,
		});
	}
});
